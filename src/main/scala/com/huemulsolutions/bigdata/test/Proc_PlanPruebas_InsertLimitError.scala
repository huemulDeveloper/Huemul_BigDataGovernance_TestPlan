...
object Proc_PlanPruebas_InsertLimitError {
  def main(args: Array[String]): Unit = {
   ...
   //BORRA HDFS ANTIGUO PARA EFECTOS DEL PLAN DE PRUEBAS
      ...
      TablaMaster.dataFrameHuemul.dataFrame.show()
      IdTestPlan = Control.RegisterTestPlan(TestPlanGroup, "Masterización", "Si hay error en masterización", "Si hay error en masterización 1005", s"Si hay error en masterización (${TablaMaster.errorCode})", TablaMaster.errorCode == 1005)
      Control.RegisterTestPlanFeature("DQ_MaxNewRecords_Num", IdTestPlan)
