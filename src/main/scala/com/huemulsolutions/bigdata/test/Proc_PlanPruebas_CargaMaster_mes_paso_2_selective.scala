...
object Proc_PlanPruebas_CargaMaster_mes_paso_2_selective {
  def main(args: Array[String]): Unit = {
    val huemulLib = new HuemulBigDataGovernance("01 - Plan pruebas Proc_PlanPruebas_CargaMaster_mes_2",args,com.yourcompany.settings.globalSettings.global)
    ...
    try {
      ...
      TablaMaster.dfFromSql("DF_Original2", s"""select 'Cero-Vacio' as TipoValor, 45 as IntValue, '$periodo_mes' as periodo_mes union all select 'Negativo_Maximo' as TipoValor, -11 as IntValue, '$periodo_mes' as periodo_mes  """)
      ...
      val conError = Control.RegisterTestPlan(TestPlanGroup, "DQ - debe tener error", "ejecuta la validación, no debe tener error", "isError = false", s"IsErorr = ${DQResultManual.isError}${DQResultManual.errorCode} ${DQResultManual.description}", !DQResultManual.isError)
      ...
      val NumDQ_conWarning = Control.RegisterTestPlan(TestPlanGroup, "DQ - N° Ejecuciones con Warning", "ejecuta la validación, debe tener 2 ejecuciones con warnings", "N° Ejecuciones = 2", s"N° Ejecuciones = ${DQResultManual.getDqResult.count { x => x.dqIsWarning }}", DQResultManual.getDqResult.count { x => x.dqIsWarning } == 2)
      ...
      val NumDQ_ErrorCode = Control.RegisterTestPlan(TestPlanGroup, "DQ - codigo error = 8", "ejecuta la validación, debe tener un error de codigo 8", "error_code = 8", s"error_code = ?", DQResultManual_Errores.getDqResult.count { x => x.dqErrorCode == 8 } == 1)
      ...
      val NumDQ_ErrorCode10 = Control.RegisterTestPlan(TestPlanGroup, "DQ - codigo error = 10", "ejecuta la validación, debe tener un error de codigo 10", "error_code = 10", s"error_code = ?", DQResultManual_Errores.getDqResult.count { x => x.dqErrorCode == 10 } == 1)
      ...
      val NumDQ_ErrorCodeSinError = Control.RegisterTestPlan(TestPlanGroup, "DQ - DecimalValue > 0.1 sin error", "ejecuta la validación, no debe tener error", "DecimalValue > 0.1 sin error", s"DecimalValue > 0.1 ?", !DQResultManual_Errores.getDqResult.filter { x => x.dqSqlFormula == "DecimalValue > 0.1" }(0).dqIsError )
      ...
      datos.show()
      Control.RegisterTestPlan(TestPlanGroup, "DQ - 2 periodos cargados", "Tablas transaction con 2 periodos cargados", "len del arreglo = 2", s"len del arreglo = ${datos.count()}", datos.count() == 2)
      ...
      if (!TablaMaster4.executeSelectiveUpdate("DF_Final",partitionNull, org.apache.spark.storage.StorageLevel.MEMORY_ONLY)) {
        ...
        IdTestPlan = Control.RegisterTestPlan(TestPlanGroup, "Masterización", "Si hay error en masterización", "si hay error en masterización (1017)", s"Si hay error en masterización (${TablaMaster4.errorCode})", TablaMaster4.errorCode == 1017)
        Control.RegisterTestPlanFeature("executeSelectiveUpdate", IdTestPlan)
        Control.RegisterTestPlanFeature("PK_NoEspecificada", IdTestPlan)
      ...
      TablaMaster5.dfFromSql("DF_Original", s"""select 'Cero-Vacio' as codTipoValor, 45 as valIntValue, '$periodo_mes' as periodo_mes2 union all  select 'Cero-Vacio' as codTipoValor, -55 as valIntValue, '$periodo_mes' as periodo_mes2 """)
      ...
      if (!TablaMaster5.executeSelectiveUpdate("DF_Final",partitionNull, org.apache.spark.storage.StorageLevel.MEMORY_ONLY)) {
        println(s"error: ${TablaMaster5.errorCode} ${TablaMaster5.errorText}")
        IdTestPlan = Control.RegisterTestPlan(TestPlanGroup, "Masterización", "Si hay error en masterización", "si hay error en masterización (1044)", s"Si hay error en masterización (${TablaMaster5.errorCode})", TablaMaster5.errorCode == 1044)
        Control.RegisterTestPlanFeature("executeSelectiveUpdate", IdTestPlan)
        Control.RegisterTestPlanFeature("DatosDuplicados", IdTestPlan)
