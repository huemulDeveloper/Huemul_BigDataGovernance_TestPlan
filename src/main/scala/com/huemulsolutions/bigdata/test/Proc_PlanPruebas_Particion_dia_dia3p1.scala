 ...
 */
object Proc_PlanPruebas_Particion_dia_dia3p1 {
  def main(args: Array[String]): Unit = {
    val huemulLib = new HuemulBigDataGovernance("01 - Proc_PlanPruebas_Particion_dia_dia2p2",args,com.yourcompany.settings.globalSettings.Global)
    ...
    try {
      ...
      IdTestPlan = Control.RegisterTestPlan(TestPlanGroup, "Resultado Ejecucion dia 03, p1", "Resultado Ejecución 01", "error = true and code = 1015", s"error = ${control_resultado01.Control_Error.ControlError_IsError} and code = ${control_resultado01.Control_Error.ControlError_ErrorCode}", control_resultado01.Control_Error.ControlError_IsError && control_resultado01.Control_Error.ControlError_ErrorCode == 1015 )

      //abre instancia de tabla para obtener algunos parámetros, solo pueden estar los datos anteriores
